				...
				<div className="bg-card border rounded-lg overflow-hidden shadow-sm">
					<Table>
						...
						<TableBody>
							{isLoading && (
								<TableRow>
									<TableCell colSpan={5} className="text-center py-4 text-gray-500">Memuat data...</TableCell>
								</TableRow>
							)}
							{!isLoading && users.map((u: any) => {
								...
								return (
									<TableRow key={u.id}>
										...
										<TableCell>
											<span className={`px-2 py-1 rounded-full text-xs font-semibold ${isPremium ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}`}>
										...
										<TableCell className="text-right space-x-2">
											...
											<Button variant="destructive" size="sm" onClick={() => handleAction(u.id, 'DELETE')} disabled={u.role === 'ADMIN'}>
												Hapus
											</Button>
										</TableCell>
									</TableRow>
							...
							{!isLoading && users.length === 0 && (
								<TableRow>
									<TableCell colSpan={5} className="text-center py-4 text-gray-500">Tidak ada pengguna ditemukan.</TableCell>
								</TableRow>
							)}
						</TableBody>
					</Table>
