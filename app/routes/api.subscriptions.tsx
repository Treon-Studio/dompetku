...
export async function action({ request, context }: ActionFunctionArgs) {
	...
	if (method === 'DELETE') {
		...
		const record = await db.subscriptions.findUnique({ where: { id: id[0] } });
