...
export default function AdminSettingsPage() {
	...
	const maintenanceFlag = (settings as Record<string, string>[]).find(
		(s: Record<string, string>) => s.key === "maintenance_mode",
	);
	const featureFlags = (settings as Record<string, string>[]).filter(
		(s: Record<string, string>) => s.key !== "maintenance_mode",
	);
					...
					<div className="p-6 space-y-6">
						...
						<div className="flex gap-4 items-end bg-gray-50 dark:bg-gray-800 p-4 rounded-lg border">
							<div className="flex-1">
								<label htmlFor="flagKey" className="text-xs font-semibold mb-1 block">
									Flag Key
								</label>
								<Input
							...
							<div className="flex-1">
								<label htmlFor="flagDesc" className="text-xs font-semibold mb-1 block">
									Description
								</label>
								<Input
									id="flagDesc"
						...
						<div className="divide-y border rounded-lg">
							...
							{featureFlags.map((flag: Record<string, string>) => (
								<div
									key={flag.key}
